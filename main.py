...
# جميع التوجيهات الأصلية بالكامل دون مساس بشخصيته
SYSTEM_PROMPT = """
...
4. أسلوب الحديث واللغة:
   ...
   - مرونة اللغات: أنت قادر على التكلم بكل اللغات. إذا تحدثت معك بلغة أخرى (مثل الإنجليزية، الصينية...) أو طلبت منك التحدث بلغة محددة (مثل اليونانية)، تجيبها وتستمر بالحديث بنفس اللغة حتى تطلب منك العودة للعربية أو تتحدث هي بالعربية مجدداً.
   - عند الدراسة والمسائل والأمور الجدية والمهمة: اشرح بكل دقة وتفصيل وعمق، وبسط الأفكار خطوة بخطوة مع التأكد من إيصال المعلومة كاملة وبوضوح تام.
   - استخدم الإيموجيات اللطيفة والدافئة دائماً ✨🌸🎀.
5. كتابة المعادلات والرموز الرياضية:
   - اكتب المعادلات والرموز الرياضية والفيزيائية بصيغة منسقة وواضحة.
"""

def clean_math_markdown(text: str) -> str:
    """
    دالة معالجة العرض: تقوم بتحويل أكواد LaTeX المعقدة التي يرسلها الذكاء الاصطناعي 
    إلى نصوص جميلة ومنسقة يفهمها تطبيق Flet ويعرضها بشكل مرتب للمستخدم.
    """
    # تحويل الكسور \frac{أ}{ب} إلى (أ / ب)
    text = re.sub(r'\\frac\{([^}]+)\}\{([^}]+)\}', r'(\1 / \2)', text)
    
    # تنظيف كلمات \text{}
    text = re.sub(r'\\text\{([^}]+)\}', r'\1', text)
    
    # تحويل الرموز اللاتينية والرياضية إلى رموز نصية جميلة
    replacements = {
        r"\\theta": "θ",
        r"\\rho": "ρ",
        r"\\tau": "τ",
...
async def main(page: ft.Page):
    ...
    def create_message_bubble(text, is_user=True):
        ...
        # تحويل نصوص LaTeX المعقدة إلى شكل منظم ومقروء قبل عرضها
        formatted_text = clean_math_markdown(text)
